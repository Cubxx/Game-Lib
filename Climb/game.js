...
{ //mobile 横屏适配
    ...
    document.body.requestFullscreen = function () {
        _full.call(document.body); screen.orientation.lock("natural");
    }
}
{
    ...
    var profile = document.getElementsByTagName('canvas')[1], ptx = profile.getContext('2d');
    profile.width = document.body.clientWidth;
    profile.height = document.body.clientHeight;
    ptx.scale(1 / scale_c, 1 / scale_c);
    var view = document.getElementsByTagName('canvas')[2], vtx = view.getContext('2d');
...
class Cell {
    ...
    crash(wall) {
        let obj = this;
        Obj_Crash(obj, wall, [
            function () { obj.spd[0] = 0; obj.left = wall.left + wall.width; },
            function () { obj.spd[1] = 0; obj.top = wall.top + wall.height; obj.isskip = false; },
            function () { obj.spd[0] = 0; obj.left = wall.left - obj.width; },
            function () { obj.spd[1] = 0; obj.top = wall.top - obj.height; obj.isskip = true; }
        ])
    }
...
{ //构建场景
    const size = [40 * scale_c, 40 * scale_c], space = [30 * scale_c, 30 * scale_c];
    var offset = [10 * scale_c, 10 * scale_c];
    var angle = 1, cells = [], poss = [];
    Array.prototype.in = function (elm) { for (let i of this) if (Math.abs(i[0] - elm[0]) < 2) return true; }
        ...
        for (let i = c; i < 2 * pi + c; i += 2 * pi / 4) {
            ...
            if (!poss.in([x, y]) && between_matrix([x, y], [-size[0], -size[1], canvas.width, canvas.height])) {
                ...
                let cell = new Cell([x, y], [
                    size[0] * (rand() + 0.3) + 10 * scale_c,
                    size[1] * (rand() + 0.1)], undefined, '#000');
                // cell.pathStyle();
                // cell.rect();
                cells.push(cell);
                construct_cell([x, y]);
...
var exit = new Cell([Math.round(rand() * (canvas.width - 10)), 0], [10 * scale_c, 10 * scale_c], undefined, '#090');
init();
{ //添加boss
    var bosses = [], boss_num = Math.round(5e-5 * view.width * view.height);
...
{ //主程序
    ...
    let expend = 0,
        scale_v = 1 / window.devicePixelRatio * (vtx.backingStorePixelRatio || vtx.webkitBackingStorePixelRatio || 1); //视角缩放
    var draw = function () {
        // try {
        n++;
        ...
        vtx.setTransform(scale_v, 0, 0, scale_v
        ...
        bosses.forEach((boss) => { //boss碰撞检测
            ...
            exit.crash(boss);
            boss.draw([[0, -100, canvas.width, canvas.height], undefined,
            function () {
        ...
        exit.draw([[0, 0, canvas.width, canvas.height], [
        ...
        player.draw([[0, 0, canvas.width, canvas.height], [
            function () { player.spd[0] = 0; player.left = 0; },
            function () { player.spd[1] = 0; player.top = 0; player.isskip = false; },

...
function Bullet(pos_initial, v_initial, type, move, obj_prop) {
    ...
    check_err([pos_initial, v_initial, this.type, this.lives, this.color, this.size, this.situation, this.situation_num]); //检查变量
    this.move = function () { this.move_func(this_obj, elm, pos_initial, v_initial, dxy); } //实参为引用类型数据，可通过形参改变实参
}

Bullet.prototype = new Maker();
Bullet.prototype.fire = function () { }
Bullet.prototype.hit = function () { this.element.style.opacity = this.lives / this.int_lives }
Bullet.prototype.dead = function () { }
Bullet.prototype.move_func = function (this_obj, elm, pos_initial, v_initial, dxy) {
    if (this.type.includes('res') && position(player.element)[1] < 150) { this.move_type = ['absorb', 500] } //回收线
    //运动函数
    var libs_func, move_num = this.move_num;
    if (this.move_type && this.move_type != false) {
        switch (this.move_type[0]) {
            case 'absorb': libs_func =
                function (r) { //res专用
            ...
            case 'shake': libs_func =
                ...
                break;
            case 'circle': libs_func =
                function (r, orient, p) { //半径 旋转方向:1顺时针 位移
                    p = p || [0, 0];
                ...
                break;
            case 'gravity': libs_func =
                function (g, v_end) {
                    var v_int = Math.sqrt(v_initial[1] ** 2 + v_initial[0] ** 2),
                        s = 530 - pos_initial[1],
                        g = g || 0.01;
                    if (v_end) { g = (v_end ** 2 - v_int ** 2) / 2 / s; }
                    dxy[1] += g;
                }
                break;
            case 'track': libs_func =
                function (a) {
                    var v_int = Math.sqrt(v_initial[1] ** 2 + v_initial[0] ** 2),
                        // dis = distance(player.element, elm),
                ...
                break;
            case 'toward': libs_func =
                function (p_end, deg, v_end_value, t, func = () => { }) {
                    p_end = p_end || [265, 265];
                    var vx = v_initial[0], vy = v_initial[1], //初速度
                        sx = p_end[0] - pos_initial[0], sy = p_end[1] - pos_initial[1]; //末位置
                    if (deg || deg === 0) {
                        ...
                        var a = sy ** 2 + sx ** 2,
                            b = 2 * sx * sy * (vx - vy),
                            c = (sy * vx) ** 2 + (sx * vy) ** 2 - (sx * v_end_value) ** 2 - 2 * vx * vy * sx * sy;
                        if (b ** 2 >= 4 * a * c) {
                            var v1x = (-b + (b ** 2 - 4 * a * c) ** 0.5) / 2 / a, v1y;
                            if (v_end_value >= Math.abs(v1x)) { v1y = (v_end_value ** 2 - v1x ** 2) ** 0.5; }
                            else { throw '运动函数错误 v1x > v_end_value\n' + v1x + '\n' + v_end_value }
                            t = 2 * sx / (vx + v1x) || 2 * sy / (vy + v1y);
                            var ax = (v1x - vx) / t,
                                ay = (v1y - vy) / t;
                        } else { throw '运动函数错误 v1x无解' }
                    }
                    else if (t) {
                        var ax = 2 * (sx - vx * t) / t ** 2,
                            ay = 2 * (sy - vy * t) / t ** 2;
                    }
                    if (t < 0) throw '运动函数错误 时间值t不属于(0, +∞)区间 ' + t + '\n参见 "../toward运动函数.xlsm"';
                ...
                break;
            default: throw 'libs未包含 ' + this.move_type[0];
        }
    }
    // debugger; //加密
    if (libs_func) { libs_func(...this.move_type.slice(1)) } //执行运动函数
    this.move_num = move_num;
    //环境类型
    var elm_sl = parseFloat(this.element.style.left),
        elm_st = parseFloat(this.element.style.top);
    var limit_l = -10, limit_t = -10, //接触外边界
        limit_r = body_cw - this.size[0] + 10,
        limit_d = body_ch - this.size[1] + 10;
    switch (this.situation) {
        case 'transfer':
            if (elm_st < limit_t || elm_st > limit_d) { elm.style.top = elm_st < limit_t ? limit_d + 'px' : limit_t + 'px'; dxy[1] = -dxy[1]; this.situation_num-- }
            if (elm_sl < limit_l || elm_sl > limit_r) { elm.style.left = elm_sl < limit_l ? limit_r + 'px' : limit_l + 'px'; dxy[0] = -dxy[0]; this.situation_num-- }
            if (this.situation_num < 0) { this.lives = 0 }
            break;
        case 'reflect':
            if (elm_st < limit_t || elm_st > limit_d) { elm.style.top = elm_st < limit_t ? limit_t + 'px' : limit_d + 'px'; dxy[1] = -dxy[1]; this.situation_num-- }
            if (elm_sl < limit_l || elm_sl > limit_r) { elm.style.left = elm_sl < limit_l ? limit_l + 'px' : limit_r + 'px'; dxy[0] = -dxy[0]; this.situation_num-- }
            if (this.situation_num < 0) { this.lives = 0 }
            break;
        case 'gravity':
            dxy[1] += 0.05;
            if (elm_st > limit_d && dxy[1] > 0) { elm.style.top = '530px'; dxy[1] = -dxy[1]; dxy[1] += 0.5; this.situation_num-- }
            if (elm_st < limit_t || this.situation_num < 0) { this.lives = 0 }
            if (elm_sl < limit_l || elm_sl >= limit_r) { elm.style.left = elm_sl < limit_l ? limit_l + 'px' : limit_r + 'px'; dxy[0] = -dxy[0]; this.situation_num-- }
            break;
        case 'bigger': //接触外边界+100
            if (elm_st < limit_t - 100 || elm_st > limit_d + 100 || elm_sl < limit_l - 100 || elm_sl > limit_r + 100) { this.lives = 0 }
            if (elm_st < -this.size[1] || elm_st > body_ch || elm_sl < -this.size[0] || elm_sl > body_cw) { //超出内边界
                if (document.body.contains(this.element)) { this.element.remove(); }
                // this.element.style.backgroundColor = '#0ff';
            } else {
                if (!document.body.contains(this.element)) { this.draw(position(this.element)); }
                // this.element.style.backgroundColor = '#000';
            }
            break;
        case 'general': //移出内边界
            if (elm_st < limit_t - 70 || elm_st > body_ch || elm_sl < -this.size[0] || elm_sl > body_cw) { this.lives = 0 }
            break;
        case 'none': break;
        default: throw 'situation 错误 ' + this.situation;
    }
    //移动
...
Bullet.prototype.crash = function (i) { //boss被撞 遍历player_bullets
...
Bullet.prototype.draw = function (p) {
    ...
    switch (this.type) {
        ...
        case 'res_bomb':
            ...
            break;
        default: throw 'bullet绘制错误:' + this.type;
    }
}
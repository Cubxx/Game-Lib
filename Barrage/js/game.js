...
var position = function (obj_elm, p) { //obj.element []位移
    var pos = p || [0, 0];
    return [parseFloat(obj_elm.style.left) + parseFloat(obj_elm.style.width) / 2 + pos[0]
...
var update = function (list, func1, func2) { //list func1有生命i func2所有i
    var survive = [];
    for (i of list) {
...
var timer = function (int_time, time_lag, func) { //定时执行 每time_lag帧执行一次
    int_time++;
...
var check_err = function (vars) { // '' [] undefined NaN null
    if (!(Array.isArray(vars))) throw 'check_err参数不是数组 ' + vars;
    for (var i = 0; i < vars.length; i++) {
        if (vars[i] == null || vars[i] === '' || Object.is(vars[i], NaN)) throw '序号 ' + (i + 1) + ' 值错误 ' + vars[i];
        if (Array.isArray(vars[i])) { if (vars[i].length != 0) { check_err(vars[i]) } else { throw '序号 ' + (i + 1) + ' 值错误 []' + vars[i] } }
    }
}
var print_log = function () { //输出数组信息
    var str = 'fps,bb,pb,b\n';
    for (i of log_list) { str += i.toString() + '\n'; }
    console.log(str)
}
var input = () => {
    var name = prompt('输入最能代表你的词 => 开启内卷模式🤏🏻\n\t\t❗仅可包含中英字母、数字、下划线、减号, 1-16位');
    if (/^[\u4E00-\u9FA5a-zA-Z0-9_-]{1,16}$/.test(name)) { return name } else { alert('兄啊这名不行👻换个名吧'); return input(); }
}
var upload = function (name) {
    axios.post("http://127.0.0.1:8080/cube_game/player/commit", { //上传数据 已替换debug
        username: name,
        score: player.score,
    }).then(respond => { console.log('上传成功👍🏻好雄！'); alert('🙉排行榜将在下一次游戏中更新') }
    ).catch(err => { console.log('上传失败捏🤡\n', err); alert('💨请按F12查看控制台输出') });
}
var download = function () {
    axios.get("http://127.0.0.1:8080/cube_game/player/top5", { //请求数据
    }).then(
        respond => {
            console.log('请求成功💪🏻开始内卷👊🏻\n');
            for (user of respond.data) {
                rank_elm.children[1].innerHTML += user.username + '<br>';
                rank_elm.children[2].innerHTML += user.score + '<br>';
            }
        }
    ).catch(err => { console.log('请求失败捏\n可能是服务器🐔了\n', err) });
}

//定义变量
var Stop, stage = new Stage(0),
    exe_time = 0, start_t, end_t, fps = 60.000, //实时帧率
    spd = 4.5, dl = 0, dr = 0, dt = 0, dd = 0,
    isfire = false, isbomb = false, bullet_clean = false, bullet_decrease = false, res_switch = true, ispause = true,
    player_bullets = [], boss_bullets = [], boss_list = [], res_list = [], log_list = [['fps', 'bb', 'pb', 'b', 'r']], //记录内存bullet数 帧率
    body_ch = document.body.clientHeight, body_cw = document.body.clientWidth,
...
for (i of audio_elm.children) { i.volume = 0.1; }

//循环体
var main = function () {
    //信息面版
    if (exe_time % 60 == 0) { start_t = Date.now() } else if (exe_time % 60 == 59) {
        end_t = Date.now(); fps = 59 * 1000 / (end_t - start_t);
        log_list.push([fps, boss_bullets.length, player_bullets.length, boss_list.length, res_list.length]);
    } exe_time++;
    if (player.invincible_time < 180) { //恢复判定
    ...
    // HAPPY END
    if (stage.level == 6 && stage.end_conditions) { //debug
    ...
    //player_bullets方法
    player_bullets = update(player_bullets, () => { i.move(); });
    //boss bullet生成
    stage.boss_generate();
    stage.bullet_generate();
    //boss方法
    boss_list = update(boss_list, () => {
        i.fire();
        i.move();
        player.wipe(i);
        player.crash(i);
    ...
    //boss_bullets方法
    boss_bullets = update(boss_bullets, () => {
        i.fire();
        i.move();
        player.wipe(i);
        player.crash(i);
    }, () => {
        if (bullet_clean) { boss_bullets.slice(-1)[0].lives -= 0.05 * boss_bullets.slice(-1)[0].int_lives; } //boss_dead
        if (bullet_decrease) { if (Math.random() > 0.9) { i.lives -= 0.5 }; } //bomb
    });
    //res方法
    res_list = update(res_list, () => {
        i.fire();
        i.move();
        player.wipe(i);
    });
...
document.onkeydown = function (e) {
    ...
    switch (e.keyCode) {
        ...
        case 32: ispause = !ispause;
            document.documentElement.requestFullscreen ? document.documentElement.requestFullscreen() : undefined; //W3C
            document.documentElement.mozRequestFullScreen ? document.documentElement.mozRequestFullScreen() : undefined; //FireFox 
            document.documentElement.webkitRequestFullScreen ? document.documentElement.webkitRequestFullScreen() : undefined; //Chrome等
            break; //space

...
{ //全局函数
    ...
    var print_log = function () { //输出数组信息
        var str = 'fps,bb,pb,b\n';
        for (let i of log_list) { str += i.toString() + '\n'; }
        console.log(str)
    }
    var input = () => {
...
for (let i of audio_elm.children) { i.volume = 0.1; }
...
var main = function () {
    ...
    //player_bullets方法
    player_bullets = update(player_bullets, (i) => { i.move(); });
    //boss bullet生成
    stage.boss_generate();
    ...
    //boss_bullets方法
    boss_bullets = update(boss_bullets, (i) => {
        i.fire();
        i.move();
        player.wipe(i);
        player.crash(i);

function Unit(elm) {
    ...
    this.elm.style.top = Math.random() * (body_height - this.height) * 2 / 3 + 50 + 'px';
...
Unit.prototype.achieve = function () {
    //运动
    var obj = this,
        spd = this.spd, elm = this.elm,
        width = this.width, height = this.height;
    var between = function (x, min, max) { return x > min && x < max; }
    var move = function () {
            ...
            if (i.elm != elm && !obj.stop) {
                var to_top = () => {
                    if (i.stop) spd[1] = -spd[1] * damp;
                        ...
                        if (spd[1] - i.spd[1] < 0) { var vh = spd[1]; spd[1] = i.spd[1] * damp; i.spd[1] = vh * damp; }
                        i.elm.style.top = i.top - (i.bottom - top) * 1 + 'px'; //向上
                    }
                ...
                var to_bottom = () => {
                    if (i.stop) spd[1] = -spd[1] * damp;
                        ...
                        if (spd[1] - i.spd[1] > 0) { var vh = spd[1]; spd[1] = i.spd[1] * damp; i.spd[1] = vh * damp; }
                        i.elm.style.top = i.top + (bottom - i.top) * 1 + 'px';
                    }
                ...
                var to_left = () => {
                    if (i.stop) spd[0] = -spd[0] * damp;
                        ...
                        if (spd[0] - i.spd[0] < 0) { var vw = spd[0]; spd[0] = i.spd[0] * damp; i.spd[0] = vw * damp; }
                        i.elm.style.left = i.left - (i.right - left) * 1 + 'px'; //向左
                    }
                ...
                var to_right = () => {
                    if (i.stop) spd[0] = -spd[0] * damp;
                        ...
                        if (spd[0] - i.spd[0] > 0) { var vw = spd[0]; spd[0] = i.spd[0] * damp; i.spd[0] = vw * damp; }
                        i.elm.style.left = i.left + (right - i.left) * 1 + 'px';
                    }
                ...
                if (between(i.left, left, right) && between(i.right, left, right)) {
                    if (crs_bottom) to_bottom(); //下包含
                    if (crs_top) to_top(); //上包含
                }
                if (between(i.top, top, bottom) && between(i.bottom, top, bottom)) {
                    if (crs_left) to_left(); //左包含
                    if (crs_right) to_right(); //右包含
                }
                if (crs_left && crs_top) { //左上角重合
                    if (crs_bottom) undefined; //左被包含
                    else if (i.right - left > i.bottom - top) to_top(); else to_left();
                }
                if (crs_right && crs_top) { //右上角重合
                    if (crs_left) undefined; //上被包含
                    else if (right - i.left > i.bottom - top) to_top(); else to_right();
                }
                if (crs_left && crs_bottom) {  //左下角重合
                    if (crs_right) undefined; //下被包含
                    else if (i.right - left > bottom - i.top) to_bottom(); else to_left();
                }
                if (crs_right && crs_bottom) { //右下角重合
                    if (crs_top) undefined; //右被包含
                    else if (right - i.left > bottom - i.top) to_bottom(); else to_right();
                }
            }
        }
        //摩擦
        ...
        //body碰撞
        if (left < 10 && spd[0] <= 0) { spd[0] = -spd[0] * damp; elm.style.left = '10px'; }
        if (right > body_width - 10 && spd[0] >= 0) { spd[0] = -spd[0] * damp; elm.style.left = body_width - width - 10 + 'px'; }
        if (top < -200 && spd[1] <= 0) { elm.style.top = body_height - height - 0 + 'px'; }
    ...
    //事件
    var url = this.url;
    this.elm.onmousedown = function (e) { //拖拽事件
        var ismove = false;
        obj.stop = true;
        spd[0] = 0; spd[1] = 0;
        ...
        document.onmousemove = function (e) {
            var ex = e.clientX || e.touches[0].clientX,
                ey = e.clientY || e.touches[0].clientY;
            var vx = e.movementX, v_limit = 10,
                vy = e.movementY;
            if (vx === undefined) vx = Math.random() * 20 - 10;
            else if (vx > v_limit) vx = v_limit; else if (vx < -v_limit) vx = -v_limit;
            if (vy === undefined) vy = Math.random() * 20 - 10;
            else if (vy > v_limit) vy = v_limit; else if (vy < -v_limit) vy = -v_limit;
            spd[0] = vx; spd[1] = vy;
            if (ex + dx > 10 && ex + dx < body_width - width - 10) elm.style.left = dx + ex + 'px';
            if (ey + dy < body_height - height - 20) elm.style.top = dy + ey + 'px';
            ismove = true;
        }
    ...
    this.elm.oncontextmenu = function () { obj.stop = true; }
...
var elms = document.getElementsByClassName('unit'), Units = [];
var gravity, f_air, f_ground, damp, vmax = 2;
var url_lic = true;
//设置面板
var btn = document.getElementById('btn');
var option = document.getElementById('option');
option.children[1].children[2].onchange = function () {
    this.previousElementSibling.innerHTML = (this.value * 100).toFixed(2); gravity = parseFloat(this.value);
}
option.children[2].children[2].onchange = function () {
    this.previousElementSibling.innerHTML = (this.value * 1e+2).toFixed(2); f_air = parseFloat(this.value);
}
option.children[3].children[2].onchange = function () {
    this.previousElementSibling.innerHTML = (this.value * 1e+3).toFixed(2); f_ground = parseFloat(this.value);
}
option.children[4].children[2].onchange = function () {
    this.previousElementSibling.innerHTML = (this.value * 1).toFixed(2); damp = parseFloat(this.value);
}
for (var child of option.children) if (child.children[2]) child.children[2].onchange();
...
var egg_key = setInterval(() => {
    ...
    for (var unit of Units) {
        if ((parseFloat(unit.elm.style.top) > body_height - 10) && true) unit.elm.remove();
    ...
    if (body.childElementCount <= 1) {
        ...
        info.onanimationend = function () { info.innerText = info.innerText.replace('...', 'BUG'); }
